...
vi.mock("../domains/files.js", async () => {
  ...
  return {
    ...
    getXspfPlaylistUrl: vi.fn((fileId) =>
      Effect.succeed(`https://api.put.io/files/${fileId}/xspf`),
    ),
    ...
    resetFileSortSettings: vi.fn(() => Effect.void),
    ...
    setFileSort: vi.fn((input) => Effect.succeed({ sorted: input })),
...
describe("sdk promise client adapters", () => {
  it("routes a broad namespace surface through the promise client", async () => {
    ...
    expect(await client.files.getXspfPlaylistUrl(4)).toBe("https://api.put.io/files/4/xspf");
