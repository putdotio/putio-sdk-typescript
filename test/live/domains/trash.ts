...
const withTrashEnabled = async <A>(runTest: () => Promise<A>): Promise<A> => {
  const settings = await authClient.account.getSettings();

  if (!settings.trash_enabled) {
    await authClient.account.saveSettings({
      trash_enabled: true,
    });
  }
...
await run("trash disposable lifecycle", () =>
  withTrashEnabled(async () => {
    ...
    try {
      ...
      probeEntries.forEach((file) => {
        assert(
          file.folder_type === "REGULAR",
          "expected trash folders to keep REGULAR folder_type",
        );
        assert(typeof file.deleted_at === "string", "expected deleted_at on trash entries");
        assert(
          typeof file.expiration_date === "string",
          "expected expiration_date on trash entries",
        );
        assert(
...
await run("trash bulk restore restores multiple top-level entries", () =>
  withTrashEnabled(async () => {
    const seed = Date.now();
    const created: number[] = [];
    const restoredIds = new Set<number>();
...
await run("trash child restore and delete reject non-toplevel entries", () =>
  withTrashEnabled(async () => {
    const seed = Date.now();
